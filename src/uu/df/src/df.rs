...
pub(crate) struct Options {
    pub(crate) show_local_fs: bool,
    pub(crate) show_all_fs: bool,
    pub(crate) human_readable: Option<HumanReadable>,
...
impl Options {
    ...
    fn from(matches: &ArgMatches) -> Result<Self, OptionsError> {
        let include: Option<Vec<_>> = matches
            .get_many::<OsString>(crate::options::OPT_TYPE)
            .map(|v| v.map(|s| s.to_string_lossy().to_string()).collect());
        let exclude: Option<Vec<_>> = matches
            .get_many::<OsString>(crate::options::OPT_EXCLUDE_TYPE)
            .map(|v| v.map(|s| s.to_string_lossy().to_string()).collect());
        ...
        Ok(Self {
            show_local_fs: matches.get_flag(crate::options::OPT_LOCAL),
            show_all_fs: matches.get_flag(crate::options::OPT_ALL),
            sync: matches.get_flag(crate::options::OPT_SYNC),
            block_size: read_block_size(matches).map_err(|e| match e {
                ...
                ParseSizeError::SizeTooBig(_) => OptionsError::BlockSizeTooLarge(
                    matches
                        .get_one::<String>(crate::options::OPT_BLOCKSIZE)
                        .unwrap()
                        .to_string(),
                ),
            ...
            header_mode: {
                if matches.get_flag(crate::options::OPT_HUMAN_READABLE_BINARY)
                    || matches.get_flag(crate::options::OPT_HUMAN_READABLE_DECIMAL)
                {
                    HeaderMode::HumanReadable
                } else if matches.get_flag(crate::options::OPT_PORTABILITY) {
                ...
                } else if matches.value_source(crate::options::OPT_OUTPUT)
            ...
            human_readable: {
                ...
                } else if matches.get_flag(crate::options::OPT_HUMAN_READABLE_DECIMAL) {
            ...
            include,
            exclude,
            show_total: matches.get_flag(crate::options::OPT_TOTAL),
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    #[cfg(windows)]
    {
        if matches.get_flag(crate::options::OPT_INODES) {
...
mod tests {
    ...
    mod is_included {
        ...
        use crate::df::{is_included, Options};
        use uucore::fsext::MountInfo;
    ...
    mod filter_mount_list {
        ...
        use crate::df::{filter_mount_list, Options};
