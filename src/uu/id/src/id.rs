...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let mut state = State {
        nflag: matches.get_flag(crate::options::OPT_NAME),
        uflag: matches.get_flag(crate::options::OPT_EFFECTIVE_USER),
        gflag: matches.get_flag(crate::options::OPT_GROUP),
        gsflag: matches.get_flag(crate::options::OPT_GROUPS),
        rflag: matches.get_flag(crate::options::OPT_REAL_ID),
        zflag: matches.get_flag(crate::options::OPT_ZERO),
        cflag: matches.get_flag(crate::options::OPT_CONTEXT),
    ...
    for i in 0..=users.len() {
        ...
        if matches.get_flag(crate::options::OPT_PASSWORD) {
            // BSD's `id` ignores all but the first specified user
            pline(possible_pw.as_ref().map(|v| v.uid));
            return Ok(());
        };
        if matches.get_flag(crate::options::OPT_HUMAN_READABLE) {
            // BSD's `id` ignores all but the first specified user
            pretty(possible_pw);
            return Ok(());
        }
        if matches.get_flag(crate::options::OPT_AUDIT) {
