...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let force_flag = matches.get_flag(crate::options::OPT_FORCE);

    // If -f(--force) is before any -i (or variants) we want prompts else no prompts
    let force_prompt_never: bool = force_flag && {
        let force_index = matches.index_of(crate::options::OPT_FORCE).unwrap_or(0);
        ![
            crate::options::OPT_PROMPT,
            crate::options::OPT_PROMPT_MORE,
            crate::options::OPT_INTERACTIVE,
        ]
        .iter()
        .any(|flag| {
    ...
    if files.is_empty() && !force_flag {
        ...
        let options = Options {
            ...
            interactive: {
                ...
                } else if matches.get_flag(crate::options::OPT_PROMPT) {
                    InteractiveMode::Always
                } else if matches.get_flag(crate::options::OPT_PROMPT_MORE) {
                    InteractiveMode::Once
                } else if matches.contains_id(crate::options::OPT_INTERACTIVE) {
                    match matches
                        .get_one::<String>(crate::options::OPT_INTERACTIVE)
                        .unwrap()
                        .as_str()
                    {
            ...
            one_fs: matches.get_flag(crate::options::OPT_ONE_FILE_SYSTEM),
            preserve_root: !matches.get_flag(crate::options::OPT_NO_PRESERVE_ROOT),
            recursive: matches.get_flag(crate::options::OPT_RECURSIVE),
            dir: matches.get_flag(crate::options::OPT_DIR),
